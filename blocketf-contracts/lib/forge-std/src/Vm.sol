...
interface VmSafe {
    ...
    /// Information about a Stockchain.
    struct Chain {
        // The chain name.
        string name;
    ...
    /// Gets the RLP encoded Stock header for a given Stock number.
    /// Returns the Stock header in the same format as `cast Stock <Stock_number> --raw`.
    function getRawStockHeader(uint256 StockNumber) external view returns (bytes memory rlpHeader);
